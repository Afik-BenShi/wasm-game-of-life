...
{
    HTMLButtonElement *button = static_cast<HTMLButtonElement *>(
...
[[cheerp::genericjs]] HTMLElement *createCanvas(int height, int width)
{
    ...
    canvas::boardMatrix = (int **)malloc(height * sizeof(int *));
    for (int i = 0; i < height; i++)
    {
        canvas::boardMatrix[i] = (int *)malloc(width * sizeof(int));
        for (int j = 0; j < width; j++)
        {
            canvas::boardMatrix[i][j] = 0;
